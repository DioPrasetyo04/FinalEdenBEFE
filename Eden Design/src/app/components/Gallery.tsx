...
export function Gallery({ language }: GalleryProps) {
  ...
  const content = {
    ID: {
      ...
      subtitle: 'Dokumentasi layanan dan fasilitas yang telah kami sediakan',
    },
    EN: {
      title: 'Our Service Gallery',
      subtitle: 'Documentation of services and facilities we have provided',
    },
  };
          ...
          {filteredImages.map((image, index) => (
            ...
            >
              ...
              <div className="relative aspect-[4/3] overflow-hidden bg-[#F9FAFB] dark:bg-[#1E293B]">
                <img
                  src={image.url}
                  alt={language === 'ID' ? image.titleID : image.titleEN}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                  <h3 className="text-white text-lg font-semibold">
                    {language === 'ID' ? image.titleID : image.titleEN}
                  </h3>
                </div>
              </div>
            </motion.div>
        ...
        {filteredImages.length === 0 && (
          ...
          >
            <p className="text-[#6B7280] dark:text-[#CBD5E1]">
              {language === 'ID' ? 'Tidak ada gambar dalam kategori ini.' : 'No images in this category.'}
            </p>
          </motion.div>
        )}
      </div>
        ...
        >
          ...
          >
            ...
            <img
              ...
              alt={language === 'ID' ? selectedImage.titleID : selectedImage.titleEN}
              className="w-full h-auto rounded-lg shadow-2xl"
            />

            {/* Title */}
            <div className="mt-4 text-center">
              <h3 className="text-white text-2xl" style={{ fontFamily: 'Playfair Display, serif' }}>
                {language === 'ID' ? selectedImage.titleID : selectedImage.titleEN}
              </h3>
            </div>
          </motion.div>
        </motion.div>
